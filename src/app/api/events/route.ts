...
export async function POST(req: NextRequest) {
  ...
  if (!title || !slug || !eventType || !startDate || !destinationId) {
    return Response.json(
      {
        error:
          "Title, slug, eventType, startDate, and destinationId are required",
      },
      { status: 400 }
    );
  }
...
export async function PUT(req: NextRequest) {
  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      await tx.eventCountry.deleteMany({
        where: { eventId: id },
      });
...
export async function DELETE(req: NextRequest) {
  ...
  return handleDelete(id, prisma.event, {
    revalidatePaths: ["/dashboard/events"],
  });
}
