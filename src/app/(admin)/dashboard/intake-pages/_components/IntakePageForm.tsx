...
const schema = z.object({
  ...
  intake: z.enum(["JANUARY", "MAY", "SEPTEMBER"]).describe("Intake is required"),
...
export function IntakePageForm({
  ...
  const [benefits, setBenefits] = useState<BenefitItem[]>(() => {
    if (initialData?.intakePageBenefits && initialData.intakePageBenefits.length > 0) {
  ...
  const [helpSteps, setHelpSteps] = useState<HelpStepItem[]>(() => {
    if (initialData?.howWeHelpJson && Array.isArray(initialData.howWeHelpJson)) {
      return initialData.howWeHelpJson;
    }
    return [
      { title: "Free Consultation", description: "Get personalized guidance on course selection and university options" },
      { title: "Application Support", description: "Complete assistance with your university applications and documents" },
      { title: "Scholarship Guidance", description: "Help identify and apply for relevant scholarship programs" },
      { title: "Visa Assistance", description: "Expert support throughout the visa application process" },
    ];
  });
  ...
  useEffect(() => {
    const load = async () => {
      try {
        // Load destinations
        const dRes = await apiClient.get<{ data: { id: string; name: string }[] }>(
          "/api/destinations",
          { limit: "1000" }
        );
        if (dRes.data) {
        ...
        // Load countries
        const cRes = await apiClient.get<{ data: { id: string; name: string }[] }>(
          "/api/countries",
          { limit: "1000" }
        );
        if (cRes.data) {
  ...
  const updateHelpStep = (index: number, field: keyof HelpStepItem, value: string) => {
    ...
    >
              ...
              <form.AppField name="destinationId">
                {(field) => (
                  <field.Select label="Destination">
                    <SelectItem value="_none_">-- Select Destination --</SelectItem>
                    {destinations.map((d) => (
              ...
              <form.AppField name="countryId">
                {(field) => (
                  <field.Select label="Country (optional)">
                    <SelectItem value="_none_">-- Global (All Countries) --</SelectItem>
                    {countries.map((c) => (
        ...
        <CardContent>
          <FieldGroup>
            ...
            <form.AppField name="heroMedia">
              {(field) => <field.Input label="Hero Media URL" placeholder="https://..." />}
            </form.AppField>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <form.AppField name="heroCTALabel">
                {(field) => <field.Input label="CTA Button Label" placeholder="Apply Now" />}
              </form.AppField>

              <form.AppField name="heroCTAUrl">
                {(field) => <field.Input label="CTA Button URL" placeholder="/apply-now" />}
              </form.AppField>
            </div>
          </FieldGroup>
        </CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Add benefit cards that will appear on the intake page. These help students understand why they should choose this intake.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <FieldGroup>
            <form.AppField name="whyChooseTitle">
              {(field) => <field.Input label="Section Title" placeholder="Why Choose January Intake?" />}
            </form.AppField>
          ...
          <div className="border-t pt-4">
            ...
            <div className="space-y-3">
              {benefits.map((benefit, index) => (
                <div key={index} className="flex gap-3 items-start p-4 bg-muted/50 rounded-lg border">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Set the deadline for this intake. A countdown timer will be displayed on the page.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <FieldGroup>
              ...
              <form.AppField name="timelineEnabled">
                {(field) => (
                  <div className="flex items-center space-x-2">
                    ...
                    />
                    <Label htmlFor="timelineEnabled">Show Countdown Timer</Label>
                  </div>
                )}
              </form.AppField>
            </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Describe the steps of how NWC helps students through the application process.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex items-center space-x-2 mb-4">
              ...
              {(field) => (
                <div className="flex items-center space-x-2">
                  <Switch
                    ...
                    onCheckedChange={(checked) => field.handleChange(checked)}
                  />
                  <Label htmlFor="howWeHelpEnabled">Show How We Help Section</Label>
                </div>
              )}
            </form.AppField>
          </div>
              ...
              <div key={index} className="flex gap-3 items-start p-4 bg-muted/50 rounded-lg border">
                ...
                <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-3">
                  <div>
                    ...
                    <Input
                      value={step.title}
                      onChange={(e) => updateHelpStep(index, "title", e.target.value)}
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => updateHelpStep(index, "description", e.target.value)}
        ...
        <CardContent>
          <FieldGroup>
            ...
            <form.AppField name="metaKeywords">
              {(field) => <field.Input label="Meta Keywords" placeholder="keyword1, keyword2, keyword3" />}
            </form.AppField>
          </FieldGroup>
        </CardContent>
      </Card>
