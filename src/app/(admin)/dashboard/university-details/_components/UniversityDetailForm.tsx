...
export function UniversityDetailForm({
  ...
  const getDefaultValues: UniversityDetailFormValues = useMemo(
    () => ({
      ...
      tuitionFees: detail?.tuitionFees || "",
      famousFor: detail?.famousFor || "",
      servicesHeading: detail?.servicesHeading || "",
      servicesDescription: detail?.servicesDescription || "",
      servicesImage: detail?.servicesImage || "",
      entryRequirements: detail?.entryRequirements || "",
      description: detail?.description || "",
      accommodation: detail?.accommodation || "",
      accommodationImage: detail?.accommodationImage || "",
    }),
    [detail]
  );
  ...
  // Watch universityId and update selectedUniversityStatus
  const universityId = form.state.values.universityId;

  useEffect(() => {
    if (!universityId) {
      setSelectedUniversityStatus(null);
      return;
    }
  ...
  useEffect(() => {
    ...
    setServicesImageUrl(detail?.servicesImage || "");
    setAccommodationImageUrl(detail?.accommodationImage || "");
  }, [getDefaultValues, form, detail]);
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        <TabsContent value="general" className="space-y-4">
          <FieldGroup>
            ...
            {selectedUniversityStatus && (
              <div className="flex items-center gap-2">
                <span className="text-sm font-medium">University Status:</span>
                <span
                  className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                    selectedUniversityStatus === "ACTIVE"
