...
import { Briefcase, MapPin, Clock, DollarSign, Calendar, CheckCircle2 } from "lucide-react";
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    ...
    description:
      ...
      `Apply for ${career.title} position at NWC Education. ${career.location || ""}`,
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-4xl">
            <div className="inline-flex items-center gap-2 bg-primary/10 text-primary px-4 py-2 rounded-full mb-4">
              ...
              <span className="text-sm font-medium">{career.department || "Career Opportunity"}</span>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-8">
              ...
              {career.responsibilities && (
                <div className="bg-card border border-border rounded-2xl p-6 md:p-8 shadow-sm">
                  ...
                  <ul className="space-y-3">
                    {parseListContent(career.responsibilities).map((item, index) => (
              ...
              {career.requirements && (
                <div className="bg-card border border-border rounded-2xl p-6 md:p-8 shadow-sm">
                  ...
                  <ul className="space-y-3">
                    {parseListContent(career.requirements).map((item, index) => (
                      <li key={index} className="flex items-start gap-3">
                        <CheckCircle2 className="w-5 h-5 text-primary shrink-0 mt-0.5" />
                        <span className="text-muted-foreground">{item}</span>
                      </li>
            ...
            <div className="lg:col-span-1">
              <div className="sticky top-24 space-y-6">
                  ...
                  <div className="space-y-4">
                    {career.jobType && (
                      <div>
                        ...
                        <div className="font-medium text-foreground">{getJobTypeLabel(career.jobType)}</div>
                      </div>
                    )}
                    {career.jobLocation && (
                      <div>
                        <div className="text-sm text-muted-foreground mb-1">Work Location</div>
                        <div className="font-medium text-foreground">{getJobTypeLabel(career.jobLocation)}</div>
                      </div>
                    )}
                    {career.location && (
                      <div>
                ...
                {/* Application Form */}
                <JobApplicationForm careerId={career.id} careerTitle={career.title} />
              </div>
            </div>
          </div>
        </div>
