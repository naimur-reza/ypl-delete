...
interface PageProps {
    params: Promise<{
        country: string;
        destination: string; // destination slug (e.g., "uk" or "study-in-uk")
        intake: string; // intake month (e.g., "january", "may", "september")
    }>;
}

// Clean destination slug (strip study-in- prefix if present)
function cleanDestinationSlug(slug: string): string {
...
export default async function CountryIntakePage({ params }: PageProps) {
    ...
    console.log('[IntakePage] Params:', { country, destination, cleanedSlug, intake });
    ...
    // Use IntakeService to get intake page data
    const intakeData = await IntakeService.getIntakePage({
        destinationSlug: cleanedSlug,
        intake: intake.toUpperCase() as IntakeMonth,
        countrySlug: country,
    ...
    return (
        <>
            ...
            {/* Application Timeline with Countdown */}
